...
<header class="max-w-xl mx-auto mt-20 text-center">
    ...
    <h2 class="inline-flex mt-2">By Lary Laracore 
        <img src="/images/lary-head.svg" alt="Head of Lary the mascot"></h2>
            ...
            <div x-data="{ show:false}" @click.away=" show = false ">
                ...
                <div x-show="show" class="py-2 absolute bg-gray-100 mt-2 rounded-xl w-full z-50" style="display:none">
                    ...
                    @foreach($categories as $category1 )
                        <a 
                            href="/category/{{ $category1->id }}"
                            class="
                                block text-left px-3 text-sm 
                                leading-6 hover:bg-blue-500 focus:bg-blue-500 
                                hover:text-white focus:text-white 
                                {{ isset($category) && $category->is($category1) ? 'bg-blue-500 text-white ' : ' '}} "
                        >{{ $category1->name }}</a>
                    @endforeach
                </div>
            </div>
